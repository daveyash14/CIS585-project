...
def sample(model, scheduler, train_config, diffusion_model_config,
           autoencoder_model_config, diffusion_config, dataset_config, vae, text_tokenizer, text_model, text_prompt):
    r"""
    ...
    text_prompt = ['He is a man with black hairs'] if text_prompt is None else text_prompt
    neg_prompt = ['She is a women.']
    empty_prompt = ['']
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        if i < 5:
            # Decode ONLY the final few images to save time
            ims = vae.decode(xt)
        else:
            ims = x0_pred
    ...
    for idx, img_tensor in enumerate(generated_images[-5:]):  # Plot the last 5 generated images
        plt.subplot(1, 5, idx + 1)
        img = torchvision.transforms.ToPILImage()(make_grid(img_tensor, nrow=1))
        plt.imshow(img)
        plt.axis("off")
        plt.title(f"Step {diffusion_config['num_timesteps'] - idx}")
    plt.tight_layout()
    plt.show()

def infer_ddpm(ddpm_config, text_prompt):
