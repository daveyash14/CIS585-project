...
def train_vqvae(config_path):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for im in tqdm(data_loader):
            ...
            # print("Quantize losses shapes:")
            # print("  Codebook Loss:", quantize_losses['codebook_loss'].shape)
            # print("  Commitment Loss:", quantize_losses['commitment_loss'].shape)
            # print("LPIPS Loss shape:", lpips_loss.shape)
            # print("Output is contiguous:", output.is_contiguous())
            # print("Input is contiguous:", im.is_contiguous())
            # print("g_loss is contiguous:", g_loss.is_contiguous())
        ...
        epoch_recon_losses.append(np.mean(recon_losses))
        epoch_perceptual_losses.append(np.mean(perceptual_losses))
        epoch_codebook_losses.append(np.mean(codebook_losses))
        epoch_gen_losses.append(np.mean(gen_losses))
        if len(disc_losses) > 0:
    ...
    plt.plot(epoch_perceptual_losses, label='Perceptual Loss')
    plt.plot(epoch_codebook_losses, label='Codebook Loss')
    # plt.plot(epoch_gen_losses, label='Generator Loss')
    if epoch_disc_losses:
        plt.plot(epoch_disc_losses, label='Discriminator Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('Training VQ-VAE Losses')
    plt.legend()
    plt.grid(True)
    # plt.savefig(os.path.join(train_config['task_name'], 'training_losses.png'))  # Save the plot
    plt.show()
